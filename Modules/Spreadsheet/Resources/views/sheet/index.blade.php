...
<section class="content no-print" style="padding: 15px 0;">
    ...
    <section class="content-header f_content-header f_product_content-header" style="margin-bottom: 25px">
        ...
        @can('create.spreadsheet')
            <button type="button" class="btn f_add-btn add_folder_btn btn-sm">
            ...
            <div class="box f_box box-solid">
                ...
                <div class="box-body">
                    <div class="row" style="display: flex;align-items: center">
                        <div class="col-md-4  col-md-offset-4 addProduct_form">
                            <div class="input-group w-full">
                                <input type="input" class="form-control" placeholder="search sheet"
                                    id="spread_sheet_tree_search">
                                {{-- <span class="input-group-addon">
                                    <i class="fas fa-search"></i>
                                </span> --}}
                            </div>
                        </div>
                        <div class="col-md-4 f_spreadsheet_expand_collapse_btn">
                            <button class="btn f_btn-primary btn-sm" id="expand_all">@lang('accounting::lang.expand_all')</button>
                            <button class="btn f_btn-primary btn-sm" id="collapse_all">@lang('accounting::lang.collapse_all')</button>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                                        ...
                                        <li>{{ $folder->name }}
                                            @can('create.spreadsheet')
                                                <span class="tree-actions">
                                                    <a class="btn btn-xs btn-default add-sheet" title="@lang('spreadsheet::lang.create_spreadsheet')"
                                                    ...
                                                    <a class="btn btn-xs btn-default edit_folder" title="@lang('spreadsheet::lang.edit_folder')"
                                                        href="#" data-name="{{ $folder->name }}"
                                                        data-id="{{ $folder->id }}">
                                                    ...
                                                    <li data-jstree='{ "icon" : "fas fa-file-excel" }'>
                                                        ...
                                                        <span class="tree-actions">
                                                            <a class="btn btn-xs btn-default text-success view-sheet"
                                                                ...
                                                                href="{{ action([\Modules\Spreadsheet\Http\Controllers\SpreadsheetController::class, 'show'], [$spreadsheet->id]) }}"
                                                                target="_blank">
                                                                <i class="fas fa-eye text-primary"></i></a>
                                                            @if (auth()->user()->id == $spreadsheet->created_by)
                                                                <a class="btn btn-modal btn-xs btn-default delete-sheet"
                                                                ...
                                                                <a data-href="{{ action([\Modules\Spreadsheet\Http\Controllers\SpreadsheetController::class, 'getShareSpreadsheet'], ['id' => $spreadsheet->id]) }}"
                                                                    title="@lang('spreadsheet::lang.share')"
                                                                    class="btn btn-default btn-xs share_excel">
                                                                    <i class="fa fa-share text-success"></i>
                                                                </a>

                                                                <a href="#" title="@lang('spreadsheet::lang.move_to_another_folder')"
                                                                    class="btn btn-default btn-xs move_to_another_folder"
                                                                    data-spreadsheet_id="{{ $spreadsheet->id }}">
                                                                    <i
                                                                        class="fas fa-external-link-alt text-warning"></i>
                                                                </a>
                                                            @endif
                                                        </span>
                                                        <div class="sheet-info">
                                                            <small class="text-muted"><strong><i class="fas fa-clock"
                                                                        title="@lang('lang_v1.updated_at')"></i></strong>
                                                                {{ @format_datetime($spreadsheet->updated_at) }}
                                                                <strong><i class="fas fa-user"
                                                                        title="@lang('business.created_by')"></i></strong>
                                                                {{ $spreadsheet->createdBy->user_full_name }}
                                                            </small>
                                                        </div>
                                                        @if (count($spreadsheet->shares) > 0)
                                                            <div class="sheet-info">
                                                                <small class="text-muted"><strong><i
                                                                            class="fas fa-share-alt"
                                                                            title="@lang('spreadsheet::lang.shared_with')"></i></strong></small>
                                                                @if (!empty($spreadsheet->shered_users))
                                                                    <small
                                                                        class="text-muted"><strong>@lang('user.users')</strong>
                                                                        -
                                                                        {{ implode(', ', $spreadsheet->shered_users) }}</small>
                                                                @endif
                                                                @if (!empty($spreadsheet->shared_roles))
                                                                    <small
                                                                        class="text-muted"><strong>@lang('user.roles')</strong>
                                                                        -
                                                                        {{ implode(', ', $spreadsheet->shared_roles) }}</small>
                                                                @endif
                                                                @if (!empty($spreadsheet->shared_todos))
                                                                    <small
                                                                        class="text-muted"><strong>@lang('spreadsheet::lang.todos')</strong>
                                                                        -
                                                                        {{ implode(', ', $spreadsheet->shared_todos) }}</small>
                                                                @endif
                                                            </div>
                                                        @endif
                                                    </li>
    ...
    <div class="modal fade" id="add_folder_modal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                {!! Form::open([
                    'action' => '\Modules\Spreadsheet\Http\Controllers\SpreadsheetController@addFolder',
                    'method' => 'post',
                ]) !!}
    ...
    <div class="modal fade" id="move_to_folder_modal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                {!! Form::open([
                    'action' => '\Modules\Spreadsheet\Http\Controllers\SpreadsheetController@moveToFolder',
                    'method' => 'post',
                ]) !!}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn f_btn-primary">@lang('spreadsheet::lang.move')</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">@lang('messages.close')</button>
                </div>
